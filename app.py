...
#app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///nogidela.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'nogidela_secret_key_change_in_production')
...
app.config['MAIL_DEFAULT_SENDER'] = os.getenv('MAIL_DEFAULT_SENDER', 'Nogidela Holdings <noreply@nogidela.co.za>')
...
BUSINESS_CONTACTS = {
    'whatsapp': os.getenv('WHATSAPP_NUMBER', '27789238433'),
    'phone_display': '+27 78 923 8433',
    'email': os.getenv('BUSINESS_EMAIL', 'info@nogidela.co.za'),
...
Nogidela Holdings Contact Form"""
        ...
        mail.send(msg)
        print(f"✅ Contact email sent for Message #{contact_message.id}")
        return True, None
...
We have received your booking request and will contact you within 2-4 hours to confirm availability and finalize details.

If you have any questions, please contact us:
📞 Phone: {BUSINESS_CONTACTS['phone_display']}
📧 Email: {BUSINESS_CONTACTS['email']}
💬 WhatsApp: {BUSINESS_CONTACTS['phone_display']}
...
Professional Services in Eastern Cape"""
        ...
        mail.send(msg)
        print(f"✅ Customer confirmation sent to {booking.client_email}")
        return True, None
...
def init_database():
    """Initialize database with default admin and seed services"""
    with app.app_context():
        db.create_all()
        
        # Create default admin if doesn't exist
        if Admin.query.count() == 0:
            admin = Admin(username='admin', email='admin@nogidela.co.za')
        ...
        if Service.query.count() == 0:
            ...
            db.session.commit()
            print("✓ Database initialized with 10 services!")
...
def home():
    ...
    return render_template('home.html', services=services, contacts=BUSINESS_CONTACTS)
...
def services():
    ...
    return render_template('services.html', categories=categories, contacts=BUSINESS_CONTACTS, today=date.today().isoformat())

@app.route('/service/<int:service_id>')
def service_detail(service_id):
    """Individual service detail page"""
    service = Service.query.get_or_404(service_id)
    return render_template('service_detail.html', service=service, contacts=BUSINESS_CONTACTS)
...
def booking_step1(service_id):
    ...
    if request.method == 'POST':
        try:
            ...
            booking = Booking(
                ...
                client_contact=request.form.get('customer_phone'),
                client_email=request.form.get('customer_email', '') or None,
    ...
    return render_template('booking_step1.html', service=service, contacts=BUSINESS_CONTACTS)
...
def contact():
    ...
    if request.method == 'POST':
        try:
            ...
            # WhatsApp backup
            contact_msg = f"""📧 NEW CONTACT MESSAGE
...
{message.message}"""
    ...
    # FAQs
    faqs = [
        {'question': 'What areas do you serve?', 'answer': 'We primarily serve the Eastern Cape region.'},
        {'question': 'How do I change or cancel my booking?', 'answer': 'Contact us via WhatsApp or email.'},
        {'question': 'Do you offer emergency services?', 'answer': 'Yes, for urgent requests call or WhatsApp directly.'},
        {'question': 'What payment methods do you accept?', 'answer': 'Cash, EFT, and other payment methods.'},
        {'question': 'How far in advance should I book?', 'answer': '3-7 days in advance, especially for weekends.'}
    ]
    
    return render_template('contact.html', faqs=faqs, contacts=BUSINESS_CONTACTS)

@app.route('/about')
def about():
    """About page"""
    return render_template('about.html', contacts=BUSINESS_CONTACTS)
...
def toggle_service(service_id):
    ...
    db.session.commit()
    flash(f'✅ {service.name} has been {"activated" if service.is_active else "deactivated"}', 'success')
    return redirect(url_for('admin_services'))
...
def admin_messages():
    ...
    messages = ContactMessage.query.order_by(ContactMessage.is_read.asc(), ContactMessage.created_at.desc()).all()
...
def inject_globals():
    ...
    return {
        ...
        'business_name': 'NOGIDELA HOLDINGS',
        'contacts': BUSINESS_CONTACTS
    }
